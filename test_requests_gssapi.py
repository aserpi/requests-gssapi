...
class GSSAPITestCase(unittest.TestCase):
    ...
    def test_handle_407(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init,
            ...
            response_ok.url = "http://www.example.org/"
            response_ok.status_code = 200
            response_ok.headers = {'proxy-authenticate': b64_negotiate_server}
            ...
            response.request = request
            response.url = "http://www.example.org/"
            response.headers = {'proxy-authenticate': b64_negotiate_token}
            response.status_code = 407
            response.connection = connection
            response._content = ""
            response.raw = raw
            auth = requests_gssapi.HTTPKerberosAuth()
            r = auth.handle_407(response)
    ...
    def test_handle_response_407(self):
        # Get a 407 from server, authenticate, and get a 200 back.
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init,
                            step=fake_resp):
            response_ok = requests.Response()
            response_ok.url = "http://www.example.org/"
    ...
    def test_handle_response_407_rejected(self):
        # Get a 407 from server, authenticate, and get another 407 back.
        # Ensure there is no infinite recursion.
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init,
                            step=fake_resp):
            connection = Mock()
